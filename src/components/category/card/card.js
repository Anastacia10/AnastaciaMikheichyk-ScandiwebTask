...
export class Card extends React.Component {
    ...
    addToCartFromCategoryPage = (product) => {
        if(product.inStock){
            const attributes = product.attributes;
            const choosenAttributes = attributes.reduce((acc, attributeSet) => {
                acc[attributeSet.name] = attributeSet.items[0].value;
                return acc;
            },{});
            this.props.addToCart(product, choosenAttributes);
        }  
    }

    render(){
        return <div className={classNames(this.s.card,{
                    [this.s.outOfStock]: !this.props.product.inStock,
                })}>
                    <NavLink to={`/product/`} onClick={() => this.props.getCurrentProductId(this.props.product.id)}>
                        <div 
                            className={this.s.card_photo}
                            style={{ background: `center / contain no-repeat url(${this.props.product.gallery[0]})`}}
                        ></div>
                    </NavLink>
                    <NavLink to={`/product/`} onClick={() => this.props.getCurrentProductId(this.props.product.id)}>
                        <div className={this.s.card_infoBlock}> 
                            <h3 className={this.s.card_name}>{`${this.props.product.name} ${this.props.product.brand}`}</h3>
                            <p 
                                className={this.s.card_price}
                            >{`
                                ${this.props.currentCurrency} 
                                ${this.props.product.prices.find((price) => {
                                    return (price.currency.symbol === this.props.currentCurrency) ? 1 : 0;
                                }).amount} 
                            `}    
                            </p>
                        </div>
                    ...
                    <button 
                        className={this.s.card_button} 
                        onClick={() => this.addToCartFromCategoryPage(this.props.product)} 
                    >
