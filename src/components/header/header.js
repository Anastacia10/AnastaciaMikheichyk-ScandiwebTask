...
export class Header extends React.Component {
  ...
  openCurrencyDetails =(e)=> {
    this.currencyRef.current.open = true;
  }

  closeCurrencyDetails =(e)=> {
    this.currencyRef.current.open = false;
  }

  openCloseMiniCart = (e) =>{
    e.preventDefault();
    if(this.miniCartRef.current.open === true){
      this.props.changeBackground();
      this.miniCartRef.current.open = false;

    }else if(this.miniCartRef.current.open === false){
      this.miniCartRef.current.open = true;
  ...
  openCloseCurrency = (e) =>{
    e.preventDefault();
    if(this.currencyRef.current.open === true){
      this.currencyRef.current.open = false;

    }else if(this.currencyRef.current.open === false){
      this.currencyRef.current.open = true;
    }
  }

  render() {
    return <div className= {this.s.container}>
      <div className= {this.s.categories}>
        {this.props.categories.map((category, index) =>
          <NavLink to={`/${category.name}`} key={index+10}>
            <button
              onClick={(e) => this.props.switchCategory(e)}
              key={index}
              className={this.props.currentCategoryName === category.name ? this.s.selected : ""}
              value={category.name}
            >
      ...
      <div className={this.s.rightBlock}>
        <details 
          ref={this.currencyRef}
          onMouseEnter={(e) => this.openCurrencyDetails(e)}
          className={this.s.currency}
        >
          <summary
          >
            <span>{this.props.currentCurrency}</span>
            <svg width="8" height="4" viewBox="0 0 8 4" xmlns="http://www.w3.org/2000/svg">
              <path d="M1 3.5L4 0.5L7 3.5" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </summary>
          <ul onMouseLeave={(e) => this.closeCurrencyDetails(e)}>
            {this.props.currencies.map((currency, index) =>{
                return <li key={index}>
                  <input 
                    type="radio" 
                    name="currency" 
                    id={`${currency.label}${index}`} 
                    value={currency.symbol}
                    onChange={(e) => this.props.switchCurrency(e)}
                    onClick={(e) => this.closeCurrencyDetails(e)}
                  />
                  <label htmlFor={`${currency.label}${index}`}>{`${currency.symbol} ${currency.label}`}</label>
                </li>
              })
            }
          </ul>
        ...
        <details
          ...
          className={this.s.header_cart}
          onClick={(e) => this.openCloseMiniCart(e)}
          open={this.state.openMiniCart}
        >
          <summary>
            ...
            <p className={this.s.header_cart__quantity}>{this.props.cart.fullQuantity}</p>
          </summary>
          <div 
            className={this.s.miniCart}
            onMouseLeave={(e) => this.openCloseMiniCart(e)}
          >
            <div className={this.s.minicart_wrapper}>
              <p className={this.s.miniCart_header}>My Bag, <span>{this.props.cart.fullQuantity} items</span></p>
              ...
              <div className={this.s.miniCart_totalPriceBlock}>
                <p>Total</p>
                <p>
                  {`
                    ${this.props.currentCurrency}
                    ${(this.props.cart.fullQuantity !== 0) ? 
                      this.props.cart.totalPrices[`${this.props.currentCurrency}`] : 0
                    }
                  `}
                </p>
              </div>
            </div>
            <div className={this.s.miniCart_buttons}>
              <NavLink to={'/cart'}>
